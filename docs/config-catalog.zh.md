...
来源：[`packages/companion/tool-companion/src/index.ts:22`](../packages/companion/tool-companion/src/index.ts)
...
- `@deepseek-ai/dsh-command-title` — requires `commands` · `sessionTitle` ([`packages/session/command-title/src/index.ts`](../packages/session/command-title/src/index.ts))
...
抽象服务类——部署加载具体的实现包而非抽象包（[capability seams](../.agents/notes/implemented/architecture/2026-06-13-capability-seams.zh.md)）。
