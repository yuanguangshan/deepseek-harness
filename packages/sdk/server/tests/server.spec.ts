    ...
    try {
      ...
      class LateAdapter extends LlmAdapter {
        override resolveModel(provider: string, model: string): Promise<LlmResolvedModelInfo> {
          return Promise.resolve({ provider, id: model, name: 'Late Provider' })
        }
      ...
      const late = setTimeout(() => {
        void ctx.llm?.registerAdapter(['late-provider'], new LateAdapter())
      }, 300)
      const server = new HarnessSdkJsonRpcServer(ctx, new FakeTransport())
...
describe('HarnessSdkJsonRpcServer session resume', () => {
  it('resumes a historical session from persistence instead of creating it fresh', async () => {
    ...
    const ctx = {
      ...
      // The historical id exists in the store (header-only listing snapshots).
      get: () => ({ list: async () => [{ header: { id: 'hist' } }, { header: { id: 'other' } }] }),
    } as unknown as Context
    const server = new HarnessSdkJsonRpcServer(ctx, new FakeTransport())
    // This isolated prompt test begins after the handshake boundary.
    ;(server as unknown as { initialized: boolean }).initialized = true
  ...
  it('creates a fresh session when the id is not persisted', async () => {
    ...
    const ctx = {
      ...
      get: () => ({ list: async () => [{ header: { id: 'other' } }] }),
    } as unknown as Context
    const server = new HarnessSdkJsonRpcServer(ctx, new FakeTransport())
    // This isolated prompt test begins after the handshake boundary.
    ;(server as unknown as { initialized: boolean }).initialized = true
