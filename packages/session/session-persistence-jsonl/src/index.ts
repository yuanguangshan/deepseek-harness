...
class JsonlSessionPersistence extends SessionPersistence {
  ...
  /**
   * Fork append fence: sessions whose log tail diverged mid-flight. The
   * in-memory seq cursor in this process no longer matches the file, so every
   * later durable write refuses until the session is resumed in a fresh
   * process. Cross-process single-writer exclusion itself is the kernel-held
   * `SessionWriteLease` — no takeover, no expiry — which supersedes this
   * fork's earlier pid-stamped ownership lock files and their per-append
   * re-verification (commit a6ab5e65fe); what remains here is the fail-loud
   * last line against a writer the lease cannot see.
   */
  private fenced = new Set<SessionId>()
  /** Log byte size this backend last wrote or observed per write-opened session. */
  ...
  async open(id: SessionId, access: SessionAccess, options?: SessionPersistenceOpenOptions): Promise<SessionHandle> {
    ...
    try {
      ...
      options?.signal?.throwIfAborted()
      // Baseline the fork append fence on the resolved artifact: the lease is
      // held from here on, so the first append must find exactly these bytes.
      await this.baselineTailBytes(id, resolved.currentPath)
   ...
   * A diverged log tail means the seq cursor this process would stamp no
   * longer matches the file; any further byte would corrupt it. Unlike the
   * superseded ownership locks this fence keeps no on-disk state: the kernel
   * lease stays with the write handle until close, and its release never
   * removes the lock file, so there is nothing to clean up here.
   * @param meta - the header naming the session's directory.
   * @param detail - what diverged.
   */
  private fenceAppend(meta: SessionHeader, detail: string): never {
  ...
  /** Refuse every durable write for a session this process fenced earlier. */
  private refuseFenced(meta: SessionHeader): void {
    if (this.fenced.has(meta.id)) {
      throw new Error(
        `session-persistence-jsonl: session "${meta.id}" is fenced in this process `
        + '(its log tail diverged earlier); resume it in a new process to continue',
      )
    }
  }

  /**
   * Baseline the fork append fence for a write-opened session: the byte size
   * the open's stable read observed, taken while the write lease is already
   * held. The first append must find exactly these bytes, or a concurrent
   * writer intervened between the open and its use.
   * @param id - the session the baseline guards.
   * @param path - the session's current-generation log path.
   */
  private async baselineTailBytes(id: SessionId, path: string): Promise<void> {
    try {
      this.tailBytes.set(id, (await stat(path)).size)
    } catch (error: unknown) {
      if (!isENOENT(error)) throw error
      // Absent at baseline time: the first append creates it from zero bytes.
      this.tailBytes.set(id, 0)
    }
  }
  ...
  private async appendLines(meta: SessionHeader, events: readonly SessionEvent[]): Promise<void> {
    ...
    try {
      ...
      if (expected !== undefined && before !== expected) {
        this.fenceAppend(meta, `log tail diverged — expected ${expected} bytes, found ${before}`)
      }
      try {
        await handle.writeFile(content)
        await handle.sync()
