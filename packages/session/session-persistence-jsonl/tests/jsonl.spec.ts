...
/**
 * Fork append-fence behavior, ported onto the upstream architecture. The
 * ownership half of the original fence (pid-stamped lock files re-verified
 * per append) is superseded by the kernel-held `SessionWriteLease`: it has no
 * takeover and no expiry, so a competing writer is refused up front instead
 * of being detected after the fact, and the backend never removes a live
 * session's lock file. What remains to pin here is the byte-tail fence
 * against a writer the lease cannot see, its stickiness, and the fresh
 * resume that clears it.
 */
describe('JsonlSessionPersistence: append fence (log tail divergence)', () => {
  let ctx: Context
  beforeEach(async () => {
    root = await freshRoot()
    ctx = new Context()
  ...
  function followOnLog(start: number): SessionEvent[] {
    return [
      { type: 'turn/start', seq: SessionSeq(start), time: 7, data: { turn: 2 } },
      { type: 'turn/end', seq: SessionSeq(start + 1), time: 8, data: { turn: 2, reason: { kind: 'completed' } } },
    ]
  }

  /** One durable foreign `turn/end` appended straight to the stored log. */
  async function growLogBehindTheLease(id: SessionId): Promise<void> {
    await appendFile(rawLogPath(root, '/work', id),
      '{"type":"turn/end","seq":6,"time":7,"data":{"turn":2,"reason":{"kind":"completed"}}}\n')
  }

  it('a competing writer cannot take the session while the holder lives', async () => {
    // Where the fork fenced a stale writer whose ownership lock had been
    // replaced, the lease refuses the competition before it can write at all.
    const m = meta('fence-lease-exclusion', '/work')
    await writeLog(ctx.sessionPersistence, m, oneTurnLog())
    const holder = await ctx.sessionPersistence.open(m.id, 'write')
    try {
      const competing = new Context()
      try {
        await competing.plugin(JsonlSessionPersistence, { root, compression: 'none' })
        await expect(competing.sessionPersistence.open(m.id, 'write'))
          .rejects.toBeInstanceOf(SessionAlreadyOwnedError)
      } finally {
        await competing.fiber.dispose()
      }
      // The holder's cursor still matches the file: it keeps appending.
      await holder.append(followOnLog(6))
      expect((await holder.read()).events.map(e => e.seq)).toEqual([0, 1, 2, 3, 4, 5, 6, 7])
    } finally {
      await holder.close()
    }
  })

  it('refuses appends when a concurrent writer grew the log, and stays fenced', async () => {
    const m = meta('fence-tail-diverged', '/work')
    await writeLog(ctx.sessionPersistence, m, oneTurnLog())
    const handle = await ctx.sessionPersistence.open(m.id, 'write')
    try {
      // The kernel lease is still this handle's; only the bytes moved. The
      // fence must not depend on lock state alone.
      await growLogBehindTheLease(m.id)
      await expect(handle.append(followOnLog(6))).rejects.toThrow(/log tail diverged/)
      // The fence is sticky: the in-memory seq cursor cannot be trusted again
      // in this process, so later appends refuse even without further divergence.
      await expect(handle.append(followOnLog(6))).rejects.toThrow(/fenced in this process/)
    } finally {
      await handle.close()
    }
  })
  ...
  it('a fresh process resumes and appends after the old writer fenced', async () => {
    const m = meta('fence-then-reopen', '/work')
    await writeLog(ctx.sessionPersistence, m, oneTurnLog())
    const fenced = await ctx.sessionPersistence.open(m.id, 'write')
    await growLogBehindTheLease(m.id)
    await expect(fenced.append(followOnLog(6))).rejects.toThrow(/log tail diverged/)
    // The way out the fence message names: a fresh process re-reads the log
    // and its own cursor, and appends again from the real tail. Closing the
    // fenced writer releases its lease the way a dead process's kernel lock
    // would.
    await fenced.close()
    const next = new Context()
    try {
      await next.plugin(JsonlSessionPersistence, { root, compression: 'none' })
      const writer = await next.sessionPersistence.open(m.id, 'write')
      await writer.append(followOnLog(7))
      await writer.close()
      const loaded = await readAll(next.sessionPersistence, m.id)
