 ...
 * 兼容两种调用：bench('en')（upstream rc.2 locale 参数）与 bench({ namespaceOnChildFiber, remoteAsService })（本地 remote.session 命名空间回归）。 */
async function bench(
  arg: 'zh' | 'en' | { namespaceOnChildFiber?: boolean; remoteAsService?: boolean; locale?: 'zh' | 'en' } = 'zh',
) {
  const locale = typeof arg === 'string' ? arg : (arg.locale ?? 'zh')
