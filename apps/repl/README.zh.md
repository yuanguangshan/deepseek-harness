...
| `/copy` | 把上一条助手回复复制到剪贴板（有代码块时取第一个，否则全文）。`Ctrl+Y` 重复复制。 |
| `/diff` | 以彩色显示工作区未暂存的 `git diff`。 |
| `/revert` | 确认后丢弃工作区全部未暂存改动（`git checkout -- .`）。 |
| `/doctor` | 体检本地依赖（PATH 上的 runtime、`git`、`rg`、`zstd`；环境变量键）并打印健康报告。 |
| `/rename <标题>` | 固定会话标题（此后停止自动起名）。 |
| `! <shell 命令>` | 本地直执行（支持管道），输出只进会话记录、不发送给模型；忙时也可用，超时 120s 强杀。 |
| `Ctrl+V` | 附上 macOS 剪贴板图片：经 `session/attach` 入库，随下一条消息发送。 |
| `@image.png`（或 `@"带空格路径.png"`） | 按路径附上磁盘图片；支持 `~` 展开。图片随下一条消息发送，同 `Ctrl+V`。 |
