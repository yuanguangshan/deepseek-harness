...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/companion/memory': { kind: 'indirect', reason: 'The store renders snapshot blocks for prompt injection; the caller (the REPL) owns model-visible placement.' },
  'packages/companion/usage': { kind: 'indirect', reason: 'The status string is rendered by the caller (the REPL status bar); nothing here reaches a model request.' },
